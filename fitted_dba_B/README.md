...
**IRF(age) = 10^(-3.27 + 0.0524 x age)/100**

nafitovano a pouzito jako odhad pro CFR na datove rady z MZ/UZIS pro ruzne vekove kategorie do 16.2.2021
```
cfr_wt(x) = (x<=29 ? db(x)*.8 : x<=39 ? db(x) : x<=49 ? db(x)*.7: x<=59 ? db(x)*.85 : x<=69 ? db(x)*1.6 : x<=79 ? db(x)*1.95 : x<=89 ? db(x)*1.25 : db(x)*.52)
```
...
```
vykresy s porovnanim hlasenych / odhadnutych na zaklade nafitovane CFR umrti
